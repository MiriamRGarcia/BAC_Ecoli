...
%TIME VARYING STIMULI/CONTROL
 ...
 inputs.IDOsol.u_interp='linearf';                                    % Stimuli definition for experiment 3:
 inputs.IDOsol.n_linear{1}=3;  
 inputs.IDOsol.n_linear{2}=5;  % OPTIONS:u_interp:
 for iexp=1:inputs.exps.n_exp                                                                   % 'sustained' |'step'|'linear'(default)|'pulse-up'|'pulse-down' 
 inputs.IDOsol.u_min{iexp}(1,:)=0*ones(1,inputs.IDOsol.n_linear{iexp});
 inputs.IDOsol.u_max{iexp}(1,:)=1*ones(1,inputs.IDOsol.n_linear{iexp});           % Minimum and maximum value for the input
 inputs.IDOsol.u_guess{iexp}(1,:)=0.5*ones(1,inputs.IDOsol.n_linear{iexp});       % Guess value for the input
 inputs.IDOsol.u_ref{iexp}=inputs.IDOsol.u_guess{iexp};                          % Reference for regularization
 inputs.IDOsol.t_con{iexp}=linspace(0,inputs.exps.t_f{iexp},inputs.IDOsol.n_linear{iexp});       % Input swithching times: Initial and final time   
 end
% 
