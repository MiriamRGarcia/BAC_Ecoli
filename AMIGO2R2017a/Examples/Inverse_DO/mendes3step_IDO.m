...
inputs.IDOsol.id_par=char('V1','Ki1','ni1','Ka1','na1','k_1','V2','Ki2',...
          'ni2','Ka2','na2','k_2','V3','Ki3','ni3','Ka3','na3','k_3',...
          'V4', 'K4','k_4','V5','K5','k_5','V6','K6','k_6',...
          'kcat1','Km1','Km2','kcat2','Km3','Km4','kcat3','Km5','Km6');                          % 'all'|User selected 
inputs.IDOsol.par_max=[  5e2  5e2 10  5e2 10  5e2  5e2  5e2  10  5e2  10  5e2  5e2 5e2...
...
inputs.IDOsol.par_min=[  1e-6   1e-6  1e-1  1e-6  1e-1  1e-6  1e-6  1e-6  1e-1  1e-6  1e-1...
  ...
  inputs.IDOsol.u_interp='sustained';               % [] Stimuli definition
  inputs.IDOsol.tcon_guess= [0 120];
  inputs.IDOsol.u_min=[0;0];
  inputs.IDOsol.u_max=[0.25;0.25];
%  inputs.IDOsol.n_steps=[2];
%  for iu=1:inputs.model.n_stimulus
%  inputs.IDOsol.u_min(iu,:)=0*ones(1,inputs.IDOsol.n_steps);
%  inputs.IDOsol.u_max(iu,:)=1*ones(1,inputs.IDOsol.n_steps);           % Minimum and maximum value for the input
%  inputs.IDOsol.u_guess(iu,:)=0.5*ones(1,inputs.IDOsol.n_steps);       % Guess value for the input
%  end
  inputs.IDOsol.u_ref=inputs.IDOsol.u_guess;    
...
inputs.IDOsol.IDOcost_type='lsq';                       % 'lsq' (weighted least squares default) | 'llk' (log likelihood) | 'user_PEcost' 
inputs.IDOsol.lsq_type='Q_expmax';                     % [] To be defined for lsq function, 'Q_I' (weighting matrix the identity | 
