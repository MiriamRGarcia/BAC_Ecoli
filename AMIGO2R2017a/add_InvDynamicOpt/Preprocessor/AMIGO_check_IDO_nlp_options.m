...
function inputs = AMIGO_check_IDO_nlp_options(inputs)

inputs_def = AMIGO_default_options_IDO;
...
if strcmpi(inputs.nlpsol.eSS.local.solver, 'nl2sol') || strcmpi(inputs.nlpsol.local.solver, 'nl2sol')
    ...
    if strcmp(inputs.IDOsol.IDOcostJac_type, 'llk')
       if ~strcmp(inputs.IDOsol.llk_type, 'homo') && ~strcmp(inputs.IDOsol.llk_type, 'homo_var') && ~strcmp(inputs.IDOsol.llk_type, 'hetero')
           ...
           error('Maximum-Likelihood estimation assuming %s noise type is not a Least-Squares problem, thus NL2SOL cannot be used.',inputs.IDOsol.llk_type);
       end
    end
end
