...
switch reopt
    ...
    case 'on'
        inputs.pathd.IDO_function=['AMIGO_IDOcost_reopt',num2str(ireopt),'_',inputs.pathd.short_name];
        inputs.pathd.IDO_constraints=['AMIGO_IDOconst_reopt',num2str(ireopt),'_',inputs.pathd.short_name];
           
end

inputs.pathd.IDO_file=fullfile(inputs.pathd.AMIGO_path,inputs.pathd.problem_folder_path,[inputs.pathd.IDO_function '.m']);
inputs.pathd.IDO_constfile=fullfile(inputs.pathd.AMIGO_path,inputs.pathd.problem_folder_path,[inputs.pathd.IDO_constraints '.m']);
...
fid2=fopen(inputs.pathd.IDO_file,'w');
fid3=fopen(inputs.pathd.IDO_constfile,'w');

fprintf(fid2,'function [f,h,g]=%s(ido,inputs,results,privstruct)\n',inputs.pathd.IDO_function);
fprintf(fid2,'%%\n%%Code generated by AMIGO to compute cost function & constraints\n%%\n');
fprintf(fid2','\n%% Solves system dynamics \n\n');


fprintf(fid2, '\t [y,f,g] = AMIGO_IDOcost(ido,inputs,results);\n');
...
if numel(inputs.model.stimulus_names)>0
    ...
    for iu=1:inputs.model.n_stimulus
        ...
        switch inputs.IDOsol.u_interp
                case {'sustained'}
                inputs.IDOsol.n_steps=1;
                for istep=1:inputs.IDOsol.n_steps{iexp}
            ...
            case {'pulse-down'}
                for istep=1:2*inputs.IDOsol.n_pulses{iexp}-1
                    fprintf(fid2,'\t ido(%u)',istep+ncount);
                    ncount2=istep+ncount;
                end
                fprintf(fid2,'\t];\n');
            ...
            case {'pulse-up'}
                for istep=1:2*inputs.IDOsol.n_pulses{iexp}
                    fprintf(fid2,'\t ido(%u)',istep+ncount);
                    ncount2=istep+ncount;
                end
                fprintf(fid2,'\t];\n');
            ...
            case {'linearf','linear'}
                for ilinear=1:inputs.IDOsol.n_linear{iexp}
...
if inputs.IDOsol.n_const_eq_tf>0
    ...
    switch inputs.nlpsol.nlpsolver
        case 'de'
            ...
            fprintf(fid2,'\nswitch inputs.nlpsol.nlpsolver\n');
            fprintf(fid2,'\ncase ''de''\n');
            for icons=1:inputs.IDOsol.n_const_eq_tf
                fprintf(fid2,'\t if abs(h(%u)) >%e\n',icons,inputs.IDOsol.eq_const_max_viol);
                fprintf(fid2,'\t f=inf;\n');
                fprintf(fid2,'\t end;\n');
            end
            fprintf(fid2,'\t end;\n');
...
end %if inputs.IDOsol.n_const_eq_tf>0
...
if inputs.IDOsol.n_const_ineq_tf>0 || inputs.IDOsol.n_const_eq_tf>0
    fprintf(fid2,'\n%% End-point inequality constraints\n');
    for icons=1:inputs.IDOsol.n_const_ineq_tf
        fprintf(fid2,'\th(%u) = %s;\n',icons+inputs.IDOsol.n_const_eq_tf,inputs.IDOsol.const_ineq_tf(icons,:));
    end
    ...
    switch inputs.nlpsol.nlpsolver
        case 'de'
            ...
            fprintf(fid2,'\ncase ''de''\n');
            for icons=1 :inputs.IDOsol.n_const_ineq_tf
                fprintf(fid2,'\t if h(%u) >%e\n',icons+inputs.IDOsol.n_const_eq_tf,inputs.IDOsol.ineq_const_max_viol);
                fprintf(fid2,'\t f=inf;\n');
                fprintf(fid2,'\t end;\n');
            end
            fprintf(fid2,'\t end;\n');
        ...
        case {'ssm','ess','eSS','fssm','local','multistart','hybrid','wsm'}
            ...
            for icons=1:inputs.IDOsol.n_const_ineq_tf
                fprintf(fid3,'problem.c_L(1,%u)=-inf;\n',icons+inputs.IDOsol.n_const_eq_tf);
                fprintf(fid3,'problem.c_U(1,%u)=%e;\n',icons+inputs.IDOsol.n_const_eq_tf,inputs.IDOsol.ineq_const_max_viol);
            end
    end
...
if inputs.IDOsol.n_const_eq_tf==0 && inputs.IDOsol.n_const_ineq_tf==0
    fprintf(fid2,'\t h(1)=0;\n');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nconstraints=inputs.IDOsol.n_const_eq_tf+inputs.IDOsol.n_const_ineq_tf;

if inputs.IDOsol.n_control_const>0
    
    
    switch inputs.exps.u_interp{1}
        case {'stepf','step'}
            
            for icons=1:inputs.IDOsol.n_control_const
                fprintf(fid2,'\t const_vector(%u,:) =%s;\n',icons,inputs.IDOsol.control_const(icons,:));
                ncount=0;
                for istep=1:inputs.IDOsol.n_steps{iexp}
                    fprintf(fid2,'\t h(%u) =const_vector(%u,%u);\n',istep+ncount+nconstraints,icons,istep);
                end
                ncount=ncount+inputs.IDOsol.n_steps;
            end
            ...
            switch inputs.nlpsol.nlpsolver
                ...
                case 'de'
                    ...
                    fprintf(fid2,'\nswitch inputs.nlpsol.nlpsolver\n');
                    fprintf(fid2,'\ncase ''de''\n');
                    for icons=1 :inputs.IDOsol.n_control_const* inputs.IDOsol.n_steps
                        fprintf(fid2,'\t if h(%u) >%e\n',icons+nconstraints,inputs.IDOsol.control_const_max_viol);
                        fprintf(fid2,'\t f=inf;\n');
                        fprintf(fid2,'\t end;\n');
                    end
                    fprintf(fid2,'\t end;\n');
        ...
        case 'linear'
            ...
            for icons=1:inputs.IDOsol.n_control_const
                fprintf(fid2,'\t const_vector(%u,:) =%s;\n',icons,inputs.IDOsol.control_const(icons,:));
                ncount=0;
                for istep=1:inputs.IDOsol.n_linear
                    fprintf(fid2,'\t h(%u) =const_vector(%u,%u);\n',istep+ncount+nconstraints,icons,istep);
                end
                ncount=ncount+inputs.IDOsol.n_linear;
            end
            ...
            switch inputs.nlpsol.nlpsolver
                ...
                case 'de'
                    ...
                    fprintf(fid2,'\nswitch inputs.nlpsol.nlpsolver\n');
                    fprintf(fid2,'\ncase ''de''\n');
                    for icons=1 :inputs.IDOsol.n_control_const* inputs.IDOsol.n_linear
                        fprintf(fid2,'\t if h(%u) >%e\n',icons+nconstraints,inputs.IDOsol.control_const_max_viol);
                        fprintf(fid2,'\t f=inf;\n');
                        fprintf(fid2,'\t end;\n');
                    end
...
end % if inputs.IDOsol.n_control_const>0


results.nlpsol.ntotalconstraints=nconstraints+ncount+inputs.IDOsol.n_pconst_ineq;
...
if inputs.IDOsol.n_pconst_ineq>0

    fprintf(fid2,'\n%% Point inequality constraints\n');
  for i=1:inputs.model.st_names
   for j=1:inputs.IDOsol.n_pconst_ineq
   fprintf(fid2,'\ty%u_tp(%u) = y(j+1,i);\n',i,j);  
  end  
  end  
  
    for icons=1:inputs.IDOsol.n_pconst_ineq
        fprintf(fid2,'\th(%u) = %s;\n',icons+nconst,inputs.IDOsol.pconst_ineq(icons,:));
    end
    ...
    switch inputs.nlpsol.nlpsolver
        case 'de'
            ...
            fprintf(fid2,'\ncase ''de''\n');
            for icons=1 :inputs.IDOsol.n_pconst_ineq
                fprintf(fid2,'\t if h(%u) >%e\n',icons+nconst,inputs.IDOsol.ineq_const_max_viol);
                fprintf(fid2,'\t f=inf;\n');
                fprintf(fid2,'\t end;\n');
            end
            fprintf(fid2,'\t end;\n');
        ...
        case {'ssm','ess','eSS','fssm','local','multistart','hybrid','wsm'}
            for icons=1:inputs.IDOsol.n_pconst_ineq
                fprintf(fid3,'problem.c_L(1,%u)=-inf;\n',icons+inputs.IDOsol.n_pconst_ineq);
                fprintf(fid3,'problem.c_U(1,%u)=%e;\n',icons+inputs.IDOsol.n_pconst_ineq,inputs.IDOsol.ineq_const_max_viol);
            end
    end
...
inputs.pathd.IDO_file
inputs.pathd.IDO_constfile
